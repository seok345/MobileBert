...
# 2. 데이터 확인
path = "fifa_10000_sampled.csv"
df = pd.read_csv(path, encoding="utf-8-sig")  # 또는 "cp949"
...
# 4. 학습용 및 검증용 데이터셋 분리 (sckit learn에 있는 train_test_split 함수 사용, random_state는 반드시 일치시킬 것)
...
# 5. MobileBERT에 영화 리뷰 데이터를 Finetuning하기 위해 데이터를 설정
# batch size는 한번에 학습하는 데이터의 양
batch_size = 8

# 학습용 테이터로 더 구현 (torch tensor
train_inputs = torch.tensor(train)
train_labels = torch.tensor(train_y)
train_masks = torch.tensor(train_mask)
train_data = TensorDataset(train_inputs,train_masks,train_labels)
tran_sampler = RandomSampler(train_data,)
train_dataloader = DataLoader(train_data, sampler=tran_sampler,batch_size = batch_size)
...
for e in range(epochs):
    ...
    train_accuracy = (np.array(train_pred) == np.array(train_true)) / len(train_pred)
    ...
    validation_accuracy = np.array((val_pred) == np.array(val_true)) / len(val_pred)

    epochs_results.append((avg_train_loss, train_accuracy, validation_accuracy))

# 9. 모델 저장
print("\n### 모델 저장 ###")
save_path = "mobilebert-custom_model_imdb"
...
for idx, (loss, train_acc, val_acc) in enumerate(epochs_results, start=1):
    print(f"Epoch {idx}: Train loss: {loss}, Train Accuracy: {train_acc}, Validation Accuracy: {val_acc}")
